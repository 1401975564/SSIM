...
class LSKblock(nn.Module):
    def __init__(self, in_channels):  # 修改初始化参数为输入通道数
        super().__init__()
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.lsk_block = LSKblock(in_chans)  # 修改LSKblock的初始化参数为输入通道数

    def forward(self, x):
        B, C, H, W = x.shape
        
        x = self.lsk_block(x)  # 使用LSKblock
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward(self, x):
        x = self.prepare_tokens(x)
        x = x + self.pos_embed[:, :1, :]
        x, mask, _ = self.random_masking(x, mask_ratio=0.01)  # 在这里调用random_masking方法
